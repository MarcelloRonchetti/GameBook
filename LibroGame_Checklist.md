...
- [x] **MapScreen — responsive scale proporzionale alla larghezza schermo**: introdotta `REFERENCE_WIDTH = 1920` + `responsiveScale = screenW / REFERENCE_WIDTH` in `MapScreen`. Tutte le dimensioni assolute (`frameW`/`tentW` dei nodi, `strokeWidth` percorsi SVG, `r` cerchi SVG, font/padding dell'header) ora sono moltiplicate per `responsiveScale`. Risultato: a 1920px tutto come calibrato, su schermi più piccoli (web in finestra, telefono) tutto rimpicciolito proporzionalmente mantenendo le stesse proporzioni relative. Risolve il problema "arches enormi" su mobile
