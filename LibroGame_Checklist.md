...
- [x] **Anagrammi cross-check completo eseguito**: verifica conteggio lettere fra ogni anagramma di scena e la sua soluzione. Tutti corretti tranne **cavallerizza** (`UNITE ZERO POSTI` = 14 lettere vs `PROSTITUZIONE` = 13 lettere — una E in più nell'anagramma). Errore presente nel PDF sorgente, non risolvibile senza inventare un nuovo anagramma. Il gameplay funziona comunque perché `checkAnagram` valida solo testo della risposta contro soluzione, non la coerenza dell'anagramma stesso. Da chiarire con l'autore. Tutti gli anagrammi della Direttrice (12) e i messaggi cifrati (intro + illusionista) verificati correttamente
